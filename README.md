...
- **Finding the pipeline**: After deploy, use the sidebar **Jobs & Pipelines** (or **Workflows** → **Pipelines**, depending on workspace UI), apply the **Pipelines** filter, and look for **`diagnostics_lab_results_pipeline`**. The deploy job also prints `databricks bundle summary`, which lists deployed resources (including pipeline IDs). Pipeline `libraries` paths must be correct relative to the YAML file that defines them (for example, under `resources/pipelines/` use `../../src/...`, not `../src/...`).
