...
- **Finding the pipeline**: After deploy, use the sidebar **Jobs & Pipelines** (or **Workflows** → **Pipelines**, depending on workspace UI), apply the **Pipelines** filter, and look for **`diagnostics_lab_results_pipeline`**. The deploy job prints **`databricks bundle summary`** and **`databricks pipelines list-pipelines`** so you can confirm the pipeline exists in the workspace API (if the list does not include it, the UI will not show it either). Pipeline definitions live under **`resources/pipelines/`** (for example **`diagnostics_lab_results_pipeline.yml`**). Root [`databricks.yml`](databricks.yml) uses **`include: resources/*.yml`** and **`resources/pipelines/*.yml`** (single-level globs) so pipeline YAML gets stable file locations for path resolution; avoid **`resources/pipelines/**/*.yml`** here if you hit deploy `Rel` errors ([CLI issue #4831](https://github.com/databricks/cli/issues/4831)). For pipeline `libraries` `file.path` values in **included** YAML, paths are resolved **relative to that YAML file** (not the bundle root); see [Databricks CLI PR #3225](https://github.com/databricks/cli/pull/3225). From `resources/pipelines/`, use **`../../src/pipelines/...`**; from `resources/`, use **`../src/pipelines/...`**.
- **Serverless pipelines and edition**: The Pipelines API requires **Advanced** edition when `serverless: true` (Core + serverless is rejected). If your workspace does not include Lakeflow Advanced, either enable that product for the account or change the pipeline to **classic compute** (`serverless: false` and a `clusters` block per the [bundle pipeline resource](https://docs.databricks.com/aws/en/dev-tools/bundles/resources#pipeline)).
